...
export class MemoryStore implements Store {
  ...
  private telemetryRow(value: NormalizedTelemetry): Record<string, unknown> { return { mark: value.mark, oracle: value.oracle, bid: value.bid, ask: value.ask, mid: value.mid, spread: value.spreadBps, funding: value.fundingRate, depth: value.depthNotional, volatility: value.volatility, block: value.block, timestamp: new Date(value.timestamp).toISOString(), source: value.source, freshness: value.freshnessMs, freshness_detail: value.freshness ?? null } }
