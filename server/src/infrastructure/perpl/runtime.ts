...
import type { Action, Book, Position } from '../../../../packages/domain/src/index.js'
...
export function createPerplRuntime(store: PostgresStore): RuntimeVenue | undefined {
  ...
  return {
    ...
    async loadBookSetup(marketId, accountId, positionId) {
      ...
      const { source: _source, freshnessMs: _freshnessMs, ...telemetrySeed } = telemetry
      ...
      return { market: market.symbol, position: positionSeed, telemetry: { ...telemetrySeed, source: telemetry.source, freshnessMs: telemetry.freshnessMs }, reserveAvailable }
    },
    async listPositions() {
      ...
      for (const market of context.markets) {
        ...
        for (const row of rows.filter(item => item.st === 1)) {
          ...
          const { bookId: _bookId, ...positionSeed } = position
          if (telemetry) { const { source: _source, freshnessMs: _freshnessMs, ...telemetrySeed } = telemetry; result.push({ marketId: market.id, market: market.symbol, accountId, positionId: row.pid, position: positionSeed, telemetry: { ...telemetrySeed, source: telemetry.source, freshnessMs: telemetry.freshnessMs } }) }
    ...
    async start() { await trading.connect() },
