...
import { defenseAmount, deriveFeatures, classifyDecision, defaultRiskConfig, evaluateManualAction, telemetryFreshnessFailures } from '../../../packages/risk-engine/src/index.js'
...
export class ExecutionWorker {
  ...
  async execute(decision: Decision, manualClose = false, manualAction = false) {
    ...
    let venueSubmissionInvoked = false
    try {
      await this.repo.addEvent({ id: crypto.randomUUID(), bookId: action.bookId, type: `${action.kind}_QUEUED`, payload: { actionId: action.id, amount: action.amount }, timestamp: new Date().toISOString() })
