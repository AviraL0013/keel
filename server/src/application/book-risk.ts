...
type Point = { status: string; updatedAt: string | null; ageMs: number | null }
...
export function evaluateBookSnapshot(book: Book, position: SnapshotPosition | null, telemetry: SnapshotTelemetry | null, reserve: SnapshotReserve | null, priorEfficiency: number, executionHealthy: boolean, now = Date.now()): Decision | null {
  ...
  const at = (point?: Point) => point?.updatedAt ? Date.parse(point.updatedAt) : Number.NaN
  const sources = telemetry.freshness
  const timestamp = sources ? Math.min(...[sources.market, sources.position, sources.funding, sources.orderbook].map(at)) : Number.NaN
