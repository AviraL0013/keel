...
export function registerRoutes(context: HttpContext) {
  ...
  app.setErrorHandler((error, _request, reply) => { logger.error({ error: error instanceof Error ? error.message : 'INTERNAL_ERROR' }, 'request failed'); const typed = error as Error & { statusCode?: number; details?: unknown }; const rateLimited = error instanceof Error && ['VENUE_HTTP_429', 'PERPL_HISTORY_HTTP_429'].includes(error.message); const status = rateLimited ? 503 : error instanceof Error && 'statusCode' in error ? Number(typed.statusCode) : 500; return reply.code(status).send({ error: rateLimited ? 'PERPL_RATE_LIMITED' : error instanceof Error ? error.message : 'INTERNAL_ERROR', ...(typed.details === undefined ? {} : { details: typed.details }) }) })
}
