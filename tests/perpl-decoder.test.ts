...
describe('Perpl protocol state',()=>{
 ...
 it('requires all snapshots before readiness',()=>{const state=new PerplStateStore();state.apply(wallet,1000);expect(state.ready(1000)).toBe(false);state.apply({mt:23,d:[]});state.apply({mt:26,d:[]});expect(state.ready(1000)).toBe(true)})
 it('merges delta positions without losing other positions',()=>{const state=new PerplStateStore();state.apply(wallet);state.apply({mt:26,d:[position(1),position(2)]});state.apply({mt:27,d:[{...position(1,50),at:{b:2,t:2000}}]});expect(state.snapshot().positions.map(p=>p.s)).toEqual([50,100]);state.apply({mt:27,d:[position(1,200)]});expect(state.snapshot().positions[0].s).toBe(50)})
