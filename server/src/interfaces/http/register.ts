...
export function registerRoutes(context: HttpContext) {
  ...
  app.setErrorHandler((error, _request, reply) => { logger.error({ error: error instanceof Error ? error.message : 'INTERNAL_ERROR' }, 'request failed'); const status = error instanceof Error && 'statusCode' in error ? Number((error as Error & { statusCode?: number }).statusCode) : 500; return reply.code(status).send({ error: error instanceof Error ? error.message : 'INTERNAL_ERROR' }) })
}
