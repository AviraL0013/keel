...
export class BooksApplication {
  ...
  async create(userId: string, command: CreateBookCommand) {
    ...
    if (this.venue?.loadBookSetup && command.marketId && command.venueAccountId && command.venuePositionId) {
      ...
      return this.books.createBook(userId, { ...command, market: setup.market, side: setup.position.side, initialPosition: setup.position, initialTelemetry: setup.telemetry, reserveAvailable, status: 'ACTIVE' })
    }
    return this.books.createBook(userId, { ...command, automationEnabled: false, reserveAvailable: command.reserveAvailable ?? 0, status: 'PAUSED' })
  }
